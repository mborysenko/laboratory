...
module SDL.Client.Resources.FileHandlers
{
	...
	{
		public static updatePaths(data: string, url: string, addHost: boolean = false): string
		{
			if (data)
			{
				var path;
				...
				return data.replace(/\{(PATH|ROOT)\}(\/?)/g, function(substring: string, token: string, next: string)
					{
						switch (token)
						{
							case "PATH":
