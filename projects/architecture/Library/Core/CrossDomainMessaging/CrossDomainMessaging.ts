...
{
	if (SDL.jQuery.inArray("origin", (<any>SDL.jQuery).event.props) == -1)
	{
		(<any>SDL.jQuery).event.props.push("origin");
	}
	if (SDL.jQuery.inArray("source", (<any>SDL.jQuery).event.props) == -1)
	{
		(<any>SDL.jQuery).event.props.push("source");
	}
	...
	export function executeMessage(message: IMessage, source: Window, origin: string)
	{
		if (message)
		{
			...
			if (message.method)
			{
				...
				else
				{
					...
					if (Type.isArray(args))
					{
						for (var i = 0, len = args.length; i < len; i++)
						{
							if (Type.isObject(args[i]) && args[i].__callbackId)
							{
	...
	function _postMessage(target: Window, message: IMessage, origin?: string)
	{
		...
		{
			target.postMessage("sdl:" + Types.Object.serialize(message), origin);
		}
	}
	...
	function _messageHandler(e)
	{
		if (e && e.data && e.data.length > 4 && e.data.indexOf("sdl:") == 0)
		{
			...
			if (allowed)
			{
				executeMessage(Types.Object.deserialize(e.data.slice(4)), e.source, e.origin);
			}
		}
	}
