...
module SDL.UI.Core.Knockout.ViewModels
{
	...
	*/
	...
	export interface IViewModelItemProperties
	{
		item: Object;
		properties: {[property: string]: IPropertyDef;};
		methods: {[method: string]: IMethodDef;};
		observables: {[event: string]: KnockoutObservable<number>;};
	}
	...
	export class ViewModelItem extends SDL.Client.Types.DisposableObject
	{
							...
							if (!(<any>p.item)[getter])
							{
								SDL.Client.Diagnostics.Assert.raiseError("Unable to determine a getter for property '" + property + "' of item " +
									((<SDL.Client.Models.IIdentifiableObject>(p.item)).getId
										? ("'" + (<SDL.Client.Models.IIdentifiableObject>(p.item)).getId() + "'")
										: ((<SDL.Client.Types.OO.IInheritable>(p.item)).getTypeName
											? (<SDL.Client.Types.OO.IInheritable>(p.item)).getTypeName()
											: "")
									) + ".");
							}
			...
			if (!(<any>item)[methodName])
			{
				SDL.Client.Diagnostics.Assert.raiseError("Method '" + methodName + "' is not defined on item " +
					((<SDL.Client.Models.IIdentifiableObject>item).getId
										? ("'" + (<SDL.Client.Models.IIdentifiableObject>item).getId() + "'")
										: ((<SDL.Client.Types.OO.IInheritable>item).getTypeName
											? (<SDL.Client.Types.OO.IInheritable>item).getTypeName()
											: "")
									) + ".");
			}
		...
		_onEvent(evt: JQueryEventObject)
		{
			var p = this.properties;
			...
			if (event == "marshal")
			{
				p.item = <any>(<SDL.Client.Models.MarshallableObject>(<any>evt.target)).getMarshalObject();
			}
